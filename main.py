...
window = Tk()
window.withdraw()
window.title('Packman Password Manager')
...
class entity_display:
    ...
    def display(self):
        ...
        self.label_pass.grid(row=6 + self.i, column=5, sticky=W)
        self.deleteButton.grid(row=6 + self.i, column=6, sticky=W)
...
submit = Button(window, text='Add Entry', command=onsubmit, font=('Courier', 14))
...
pass_label2.grid(row=5, column=5)
